...
In this section, we will see how **imgLP** can be installed in your Python projects.
...
Here, we want to create a new Python project with a dependency on **imgLP**.
...
Then, we just need to add the dependencies to your project, typically dependency to **imgLP**, with the command:

.. code-block:: shell

   uv add imglp

You can redo this command for all the dependencies you might need by replacing the "imgLP" by the other dependency name (the name ususally used in the command ``pip``).
...
.. tab-set::
   ...
   .. tab-item:: Anaconda tools (Spyder Jupyter)
      ...
      To be able to use another virtual environment, *Spyder* and *Jupyter* connects to specific kernels, respectively *spyder-kernels* and *ipykernel*, which are included in **imgLP** library dependencies.
...
To finish, some scripts may be provided with the library **imgLP** :doc:`(see Reference Guide) <scripts>`.
To use them, you can just download `scripts source codes <https://github.com/LancelotPincet/imgLP/tree/main/src/imglp/scripts>`_.
...
.. dropdown:: Using *Anaconda* distribution
   ...
   *Anaconda* is a Python distribution designed for scientific programming.
   ...
   Here we will explain how to setup up virtual environments dependent on **imgLP** that you will be able to use on various projects.
   ...
   However some dependencies for this Python project might not be available in this *base* environment, like **imgLP**.
   ...
   - create venv: ``conda create -n venv_imgLP python``
   - activate venv: ``conda activate venv_imgLP``
   - update pip: ``conda install pip``
   - add *Spyder* and *Jupyter* kernels: ``conda install spyder-kernels ipykernel``
   - install **imgLP**: ``pip install imglp``
...
.. dropdown:: Manual project
   ...
   6. Install **imgLP** from terminal: ``pip install imglp``
...
If ever you want to play with the library source code, to apply your own modifications, or even to contribute, you can access it on the `GitHub page <https://github.com/LancelotPincet/imgLP>`_.
...
Now, you can clone in your current directory the `source code folder <https://github.com/LancelotPincet/imgLP>`_ with the following command
