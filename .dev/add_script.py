...
def main() :
    ...
    while already_exists :
        ...
        script_path = path.parent / f'libsLP/imgLP/src/imglp/scripts/{name}_LP'
        module_path = path.parent / f'libsLP/imgLP/src/imglp/modules/{name}_LP'
    ...
    def copy_file(from_path, to_path) :
        ...
        string = string.replace('template_scriptlib', "imgLP")
        string = string.replace('template_scriptlowerlib', "imglp")
        string = string.replace('template_scriptequals', "="*len(name))
        string = string.replace('template_scrlibeq', "="*len("imgLP"))
    ...
    toml_path = path.parent / f'libsLP/imgLP/pyproject.toml'
    with open(toml_path, "r") as file:
        data = toml.load(file)
    data.setdefault("project", {}).setdefault("scripts", {})[name] = f"imgLP.scripts.{name}_LP.{name}:{name}"
    ...
    scrjson['date'] = date
    json_path = path.parent / f'libsLP/imgLP/src/imglp/scripts.json'
    ...
    newrst_path = path.parent / f'libsLP/imgLP/docs/source/{name}.rst'
    copy_file(rst_path, newrst_path)

    allscripts_path = path.parent / f'libsLP/imgLP/docs/source/scripts.rst'
